...
class SlideShow {
  constructor() {
    ...
    slideshows.forEach((slideshow) => {
      ...
      if(slideshow.classList.contains("mobile-only") && document.body.clientWidth >= 768){
        return true;
      }

      console.log(slideshow, "jQuery(slideshow).parents('.carousel').length: ", jQuery(slideshow).parents('.carousel').length)

      if(jQuery(slideshow).parents('.carousel').length > 0){
        return true;
      }

      // handle sub carousel slides
      jQuery(slideshow).find(".carousel__slide .carousel__slide").removeClass('carousel__slide')

      let pageDots = false;
      if(slideshow.getAttribute('data-dots') == "enabled"){
        pageDots = true
      }
