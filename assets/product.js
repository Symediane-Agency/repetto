  ...
  class VariantSelects extends HTMLElement {
    ...
    toggleAddButton(disable = true, text = false, modifyClass = true) {
      ...
      submitButtons.forEach((submitButton) => {
        ...
        if (submitButton.classList.contains('cwip')) {
    ...
    setDisabled() {
      ...
      if (variant_data) {
        ...
        // console.log('AVAILABLE OPTIONS: ', available_options)
        ...
        this.fieldsets.forEach((fieldset, i) => {
          ...
          if (fieldset_options) {
            if (fieldset.querySelector('select')) {
              ...
              fieldset_options.forEach((option, option_i) => {
                if (option.isUnavailable) {
                  fieldset.querySelector('input[value=' + JSON.stringify(option.value) + ']').classList.add('is-disabled');
                }
  ...
  class ProductSlider extends HTMLElement {
    ...
    setEventListeners(container, activeImages) {
      let links = container.querySelectorAll('.product-single__media-zoom'),
        zoomerlinks = container.querySelectorAll('.product-single__media-zoomer'),
      ...
      zoomerlinks.forEach((zoomerlink => {
        zoomerlink.addEventListener('click', (event) => {
          let i = zoomerlink.dataset.index;

          options.index = parseInt(i, 10);
          options.getThumbBoundsFn = () => {
            const thumbnail = zoomerlink.closest('.product-single__media'),
              pageYScroll = window.pageYOffset || document.documentElement.scrollTop,
              rect = thumbnail.getBoundingClientRect();
